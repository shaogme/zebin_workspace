...
fn test_native_container_round_trip_some() {
    ...
    let mut reader_obj = zebin::reader::<NativeContainers, _>(&buf).unwrap();
...
fn test_borrowed_container_round_trip() {
    ...
    let buf = zebin::encode(value).unwrap();
    let mut reader_obj = zebin::reader::<BorrowedContainers, _>(&buf).unwrap();
    let archived = reader_obj.read().unwrap();
    ...
    let borrowed: Vec<u32> = archived.borrowed_numbers.deserialize().unwrap();
    assert_eq!(borrowed, vec![1, 2, 3]);
    let owned: Vec<u32> = archived.owned_numbers.deserialize().unwrap();
