...
mod alloc_tests {
    ...
    fn test_binary_heap_direct() {
        ...
        let deserialized: BinaryHeap<i32> = zebin::deserialize::<BinaryHeap<i32>, _>(&bytes)
            .expect("failed to deserialize BinaryHeap");

        let deserialized_sorted: Vec<i32> = deserialized.into_sorted_vec();
        let expected_sorted: Vec<i32> = heap.into_sorted_vec();
        assert_eq!(deserialized_sorted, expected_sorted);
    }
    ...
    fn test_btreemap_direct() {
        ...
        let deserialized: BTreeMap<String, u32> =
            zebin::deserialize::<BTreeMap<String, u32>, _>(&bytes)
...
mod std_tests {
    ...
    fn test_hashset_direct() {
        ...
        let deserialized: HashSet<String> = zebin::deserialize::<HashSet<String>, _>(&bytes)
            .expect("failed to deserialize HashSet");
        assert_eq!(deserialized, set);
    }
    ...
    fn test_hashmap_direct() {
        ...
        let deserialized: HashMap<u32, String> =
            zebin::deserialize::<HashMap<u32, String>, _>(&bytes)
    ...
    fn test_collections_container() {
        ...
        let deserialized: CollectionContainer =
            zebin::deserialize::<CollectionContainer, _>(&bytes)
