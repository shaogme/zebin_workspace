...
fn test_varint_struct_round_trip() {
    ...
    let mut reader_obj = zebin::reader::<VarIntStruct, _>(&buf).unwrap();
...
fn test_varint_usize() {
    ...
    let buf = zebin::encode(s).unwrap();
    let mut reader_obj = zebin::reader::<NestedVarInt, _>(&buf).unwrap();
    let archived = reader_obj.read().unwrap();
...
fn test_varint_enum_round_trip() {
    ...
    let mut reader_obj = zebin::reader::<VarIntEnum, _>(&buf).unwrap();
    let archived = reader_obj.read().unwrap();
...
fn test_varint_in_varint_vec() {
    ...
    let mut reader_obj = zebin::reader::<VarIntVec<u32>, _>(&buf).unwrap();
    let archived = reader_obj.read().unwrap();
