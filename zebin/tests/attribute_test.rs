...
fn test_rename_and_skip() {
    ...
    let mut reader_obj = zebin::reader::<AttributeTest>(&buf).unwrap();
...
fn test_tuple_skip() {
    ...
    let mut reader_obj = zebin::reader::<TupleTest>(&buf).unwrap();
    let archived = reader_obj.read().unwrap();
...
fn test_enum_skip_rename() {
    ...
    let mut reader_obj1 = zebin::reader::<EnumTest>(&buf1).unwrap();
    ...
    let mut reader_obj2 = zebin::reader::<EnumTest>(&buf2).unwrap();
