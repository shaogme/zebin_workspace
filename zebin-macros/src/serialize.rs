...
fn record_state_input_impl(
    ...
    quote! {
        impl<'a> zebin::io::Serializer for #state_name<'a> {
            type Input = #input_ty;
            fn input(&mut self, item: Self::Input, sink: &mut zebin::io::CursorMut<'_>) -> Result<::core::task::Poll<()>, zebin::ZebinError> {
            ...
            fn poll_pending(&mut self, serializer: &mut zebin::io::CursorMut<'_>) -> Result<::core::task::Poll<()>, zebin::ZebinError> {
                #logic
            }
            fn finish(self, sink: &mut zebin::io::CursorMut<'_>) -> Result<::core::task::Poll<()>, zebin::ZebinError> {
                #(#finishes)*
                Ok(::core::task::Poll::Ready(()))
            }
        }
